...
nullspace_riccati_data::nullspace_riccati_data(problem_ptr_t prob)
    : node_data(prob), nx(prob->dim_[__x]), nu(prob->dim_[__u]),
      ...
      Q_xu(raw_data_.hessian_[__x][__u]), Q_uu(raw_data_.hessian_[__u][__u]),
      Q_xy(raw_data_.hessian_[__x][__y]), Q_yy(raw_data_.hessian_[__y][__y]) {
    nz = nu - ncstr;
    nsp_ = new nullspace_data(raw_data_.approx_[__eq_cstr_s].jac_[__y]);
