...
void ns_sqp::update(size_t n_iter) {
    { // rollout for initialization
        ...
        graph_.apply_all_unary_parallel([this](solver::data_base *cur) {
            // setup settings
            for(auto field : range_n(__dyn, field::num_constr)) {
                for (const auto &c : cur->prob_->expr_[field]) {
                    auto &constr = static_cast<impl::constr &>(*c);
                    constr.setup_solver_setting(cur->data(constr), &settings);
                }
            }
    ...
    for ([[maybe_unused]] size_t i_iter : range(n_iter)) {
        ...
        auto finalize_bound = [&]() {
            for (size_t i = 0; i < get_num_threads(); ++i) {
                settings.primal.merge_from(ls_config[i].primal);
                settings.dual.merge_from(ls_config[i].dual);
            }
