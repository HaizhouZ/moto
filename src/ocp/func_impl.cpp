...
void func::substitute(sym &arg, sym &rhs) {
...
void func::set_from_casadi(std::initializer_list<expr_ref> in_args, const cs::SX &out) {
...
std::future<void> func_codegen::make_codegen_task(func *f) {
    ...
    t.append_value = f->field() == __cost;
    t.append_jac = f->field() == __cost;
    t.verbose = true;
