...
struct srbd_dynamics : public dynamics {
    ...
    expr_list friction_cone(scalar_t mu = 0.5) {
        ...
        return {constr("fric_l", {f_l}, make_fric_cone(f_l)).as_ineq(),
                constr("fric_r", {f_r}, make_fric_cone(f_r)).as_ineq()};
    }
    expr_list foot_loc_constr() {
        return {constr("foot_overlap", {r_l_n, r_r_n}, cs::SX::norm_2(r_l_n - r_r_n) - 0.16).as_ineq()}; // 4cm foot
    }
    expr_list stance_foot_constr() {
        return {constr("stance_l",
                       {v_l, f_l, r_l, active_l},
                       cs::SX::vertcat({active_l * v_l, (1 - active_l) * f_l, active_l * r_l(2)}))
                    .as_eq(),
                constr("stance_r",
                       {v_r, f_r, r_r, active_r},
                       cs::SX::vertcat({active_r * v_r, (1 - active_r) * f_r, active_r * r_r(2)}))
                    .as_eq()};
    }
    expr_list running_cost() {
        ...
        return {cost("running_cost", {r, v, f_l, f_r, v_l, v_r, r_d}, dt * running_cost)};
    }
    expr_list terminal_cost() {
        ...
        return {cost("terminal_cost", {r, v, r_l, r_r, r_d}, running_cost)};
    }
};
