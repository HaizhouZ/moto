...
class QuadrupedModel(ContactRobotModel):
    def __init__(
        ...
        lifted_contact: bool = True,
        lifted_acceleration: bool = False,
    ):
        super().__init__(
            model,
            name=name,
            ...
            lifted_acceleration=lifted_acceleration,
        )
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "legacy explicit contact-force input with semi-implicit Euler, "
            "RNEA dynamics, and explicit contact constraints"
        ),
    )
    parser.add_argument(
        "--lifted-acceleration",
        action="store_true",
        help="also lift acceleration as a comparison formulation",
    )
    parser.add_argument(
        "--max-iter",
        type=int,
    ...
    if args.acceleration_control and args.explicit_rnea_contact:
        parser.error(
            "--acceleration-control and --explicit-rnea-contact are exclusive"
        )
    ...
    model = QuadrupedModel(
        ...
        lifted_contact=(
            not args.acceleration_control and not args.explicit_rnea_contact
        ),
        ...
        + (
            ...
            if args.acceleration_control
                ...
                if args.explicit_rnea_contact
                    ...
                    else "lifted RNEA/contact + semi-implicit Euler"
                )
            )
        )
    )
    ...
    def build_stage_prob(robot: QuadrupedModel):
        ...
        if robot.lifted_contact:
            stage_prob.add(robot.contacts.friction_constraints)
        else:
    ...
    def create_phase_config(step):
        constr_to_disable = []
        phase_terms = (
            model.contacts.friction_constraints
            if model.lifted_contact
            else model.contacts.kinematic_constraints
        )
        for idx, f in enumerate([0, 3, 1, 2]):
            if step % 2 == 0:
                if not gait_setting[idx]:
                    constr_to_disable.append(phase_terms[f])
                    if not args.acceleration_control and not model.lifted_contact:
                        constr_to_disable.append(model.contacts.friction_constraints[f])
            else:
                if gait_setting[idx]:
                    constr_to_disable.append(phase_terms[f])
                    if not args.acceleration_control and not model.lifted_contact:
    ...
    def gait_setup(data: moto.sqp.data_type, node_index):
        ...
        if model.lifted_contact:
            active = np.ones(4)
            if ref_step > 0:
                for idx, foot in enumerate([0, 3, 1, 2]):
                    swing = (
                        not gait_setting[idx]
                        if ref_step % 2 == 0
                        else bool(gait_setting[idx])
                    )
                    if swing:
                        active[foot] = 0.0
    ...
    sys.stdout.flush()
    print(
        f"result: {res.result}, solved={res.solved}, iterations={res.num_iter}"
    )
