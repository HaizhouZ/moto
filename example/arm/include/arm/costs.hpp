...
struct armCosts {
    ...
    struct state_cost : public cost {
        vector d_q, d_v;
        state_cost(sym q, sym v) : cost("dI_state_cost") {
            d_q.resize(7);
            d_q.setConstant(10);
            d_v.resize(7);
            d_v.setConstant(0.1);
    ...
    static expr_list running(sym q, sym v, sym a) {
        return {state_cost(q, v), input_cost(a), ee_cost(q)};
    }
    static expr_list terminal(sym q, sym v) {
        return {state_cost(q, v).as_terminal(), ee_cost(q).as_terminal()};
    }
};
