 ...
 */
struct sym : public cs::SX, public impl::shared_handle<impl::expr, sym> {
    using shared_handle::operator->; // inherit operator-> to access expr methods
    using shared_handle::get;
    /**
    ...
    sym(const std::string &name, size_t dim, field_t type)
        : shared_handle(new expr_type(name, dim, type)), cs::SX(cs::SX::sym(name, dim)) {
