...
namespace moto {
namespace solver {
/// @brief Soft equality constraint via proximal augmented Lagrangian (no
/// slack):
...
///              update_approximation() separately adds J^T*lambda from the base
///              soft_constr contribution
///   Hessian:   +=(1/rho) * J^T * J  (via lag_hess_)
/// Dual update from row 2: dlam = (J*du + h) / rho
/// As rho -> 0: (1/rho)*J^T*J penalty dominates, forcing J*du = -h (hard
/// constraint recovery). As rho -> inf: penalty vanishes, soft constraint has
/// negligible effect.
class pmm_constr : public soft_constr {
  ...
  public:
    struct approx_data : public base::approx_data {
        vector g_;                  ///< raw constraint residual h = C(x), snapshot of v_ after
               ///< value_impl
  ...
  vector_ref jacobian_step(data_map_t &data) const override;
    ...
    func_approx_data_ptr_t create_approx_data(sym_data &primal, lag_data &raw, shared_data &shared) const override {
        std::unique_ptr<base::approx_data> base_d(
