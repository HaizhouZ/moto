...
def build_infeasible_sqp():
    ...
    init_eq = moto.constr.create("probe_infeas_x0_eq", [x], x.sx)
    u_zero = moto.constr.create("probe_infeas_u_eq", [u], u.sx)
    term_eq = moto.constr.create("probe_infeas_xN_eq", [x], x.sx - 1.0)
    dyn = moto.dense_dynamics.create("probe_infeas_integrator_dyn", [x, xn, u], xn.sx - x.sx - u.sx)
    running_cost = moto.cost.create(
        "probe_infeas_running_cost",
        [x, u],
        1e-3 * cs.sumsqr(x.sx) + 1e-3 * cs.sumsqr(u.sx),
    ).set_diag_hess()
    terminal_cost = moto.cost.create(
        "probe_infeas_terminal_cost",
        [x],
        1e-3 * cs.sumsqr(x.sx),
    ).set_diag_hess()
