...
TEST_CASE("exprOwnership") {
    ...
    REQUIRE(c->in_args().size() == 2); // Function should have 2 input argument
    ...
    REQUIRE(d->in_args().size() == c->in_args().size()); // Function d should have same input arguments as c
    {
        size_t d_arg_idx = 0;
        for (sym &arg : d->dep()) {
    ...
    REQUIRE(p->in_args().size() == c->in_args().size() + 1); // Function p should have one more input argument than c
